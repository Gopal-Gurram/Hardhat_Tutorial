...
describe("Token contract", function () {
  ...
  it("should fail if the sender doesn't have enough tokens", async function () {
    ...
    expect(await hardhatToken.connect(addr1).transfer(owner.address,1)).to.revertedWith("Not enough tokens");
    // owner balances shouldn't have changed
    expect(await hardhatToken.balanceOf(owner.address)).to.equal(initialBalance);
  });
  it("should update the balances after transfer the tokens" , async function () {
    const initialBalance = await hardhatToken.balanceOf(owner.adddress);
    // transfer 100 tokens from owner to addr1
   await hardhatToken.transfer(addr1.address,100);
    // transfer 50 tokens from owner to addr1

   await hardhatToken.transfer(addr2.adddress,50);
